...
-- precentage of fitness completion
...
select count(*) from exercise_completion ec,sessions s
where ec.session_id = s.session_id
and s.session_status = 'ACTIVE'
and ec.execution_date >= 
(SELECT DATE_SUB(CURDATE(), INTERVAL DAYOFWEEK(CURDATE()) - 1 DAY) AS start_of_week);

select sum( pe.time_duration * (case pe.time_unit when'Weekly'then 1 else 7 end ))
from plan_exercises pe,sessions s
where pe.session_id = s.session_id
and s.session_status = 'ACTIVE';
